...
export const getCustomers = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const allowedSortFields = new Set(['createdAt', 'totalAmount', 'orderCount', 'lastOrderDate', 'name']);
    const safeSortField =
    ...
    if (registrationDateFrom !== undefined) {
      if (typeof registrationDateFrom !== 'string')
        return next(new BadRequestError('Некорректный registrationDateFrom'));
      const d = new Date(registrationDateFrom);
      if (Number.isNaN(d.getTime())) return next(new BadRequestError('Некорректный registrationDateFrom'));
      filters.createdAt = { ...filters.createdAt, $gte: d };
    }

    if (registrationDateTo !== undefined) {
      if (typeof registrationDateTo !== 'string')
        return next(new BadRequestError('Некорректный registrationDateTo'));
      const endOfDay = new Date(registrationDateTo);
      if (Number.isNaN(endOfDay.getTime())) return next(new BadRequestError('Некорректный registrationDateTo'));
    ...
    if (lastOrderDateFrom !== undefined) {
      if (typeof lastOrderDateFrom !== 'string')
        return next(new BadRequestError('Некорректный lastOrderDateFrom'));
      const d = new Date(lastOrderDateFrom);
      if (Number.isNaN(d.getTime())) return next(new BadRequestError('Некорректный lastOrderDateFrom'));
    ...
    if (lastOrderDateTo !== undefined) {
      ...
      if (Number.isNaN(endOfDay.getTime())) return next(new BadRequestError('Некорректный lastOrderDateTo'));
      endOfDay.setHours(23, 59, 59, 999);
    ...
    if (orderCountFrom !== undefined) {
      ...
      if (!Number.isFinite(n)) return next(new BadRequestError('Некорректный orderCountFrom'));
      filters.orderCount = { ...filters.orderCount, $gte: n };
    }

    if (orderCountTo !== undefined) {
      const n = Number(orderCountTo);
      if (!Number.isFinite(n)) return next(new BadRequestError('Некорректный orderCountTo'));
